...
export const TracingBeam = ({ children, className }: { children: React.ReactNode; className?: string }) => {
  ...
  return (
    <motion.div ref={ref} className={cn('relative mx-auto h-full w-full', className)}>
      <div className="absolute -left-6 -top-6 md:-left-5 3xl:-left-16">
        <motion.div
          transition={{
            duration: 0.2,
            delay: 0.5,
        ...
        >
          <motion.div
            ...
            animate={{
              backgroundColor: scrollYProgress.get() > 0 ? 'white' : '#6d28d9',
              borderColor: scrollYProgress.get() > 0 ? 'white' : '#6d28d9',
            }}
            className="h-2 w-2  animate-pulse rounded-full border border-neutral-300 bg-white"
          />
        </motion.div>
        <svg
